...
TELEGRAM_BOT_TOKEN = '7774369692:AAGLVH2RJ7M2cGM3UW_olTC0HA5iquAL3Ms'
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Lawde madarchod bhosdike You need to be approved to use this bot.*", parse_mode='Markdown')
        return

    if attack_in_progress:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Lawde Another attack is already in progress. Please wait.*", parse_mode='Markdown')
        return
